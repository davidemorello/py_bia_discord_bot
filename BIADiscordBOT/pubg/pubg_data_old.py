...
async def get_board(players_stats, main_field, list_extra, ismean=True):
    ...
    for player in players_stats:
        ...
        if rounds_played > min_played_match:
            if ismean:
...
async def get_winners(players_stats):
    ...
    for player in players_stats:
        if players_stats[player].get("roundsPlayed") > min_played_match:
...
async def get_killerboards(players_stats):
    ...
    for player in players_stats:
        if int(players_stats[player].get("roundsPlayed")) > min_played_match:
